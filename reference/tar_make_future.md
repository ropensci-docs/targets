...
- workers:
  ...
  Positive integer, maximum number of transient `future` workers allowed
...
[`tar_make()`](https://docs.ropensci.org/targets/reference/tar_make.md)
except that targets run in parallel with transient `future` workers. It
requires that you declare your
[`future::plan()`](https://future.futureverse.org/reference/plan.html)
inside the target script file (default: `_targets.R`). `future` is not a
strict dependency of `targets`, so you must install `future` yourself.

To configure `tar_make_future()` with a computing cluster, see the

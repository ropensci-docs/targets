...
Unset (i.e. delete) one or more custom settings for the current project
from the optional YAML configuration file. After that,
[`tar_option_get()`](https://docs.ropensci.org/targets/reference/tar_option_get.md)
will return the original default values for those settings for the
project.
...
- names:
  ...
  Character vector of configuration settings to delete from the current
  project.

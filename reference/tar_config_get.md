...
Read the custom settings for the current project in the optional YAML
configuration file.
...
- name:
  ...
  Character of length 1, name of the specific configuration setting to
  retrieve.
...
- config:
  ...
  configuration file that `tar_config_get()` reads from or
...
- project:
  ...
  call to
  [`tar_config_set()`](https://docs.ropensci.org/targets/reference/tar_config_set.md)
  or `tar_config_get()`. The default project is always called `"main"`
  unless you set another default project using the `TAR_PROJECT`
  environment variable, e.g. `Sys.setenv(tar_project = "custom")`. This
  also has the effect of temporarily modifying the default arguments to
...
The value of the configuration setting from the YAML configuration file
(default: `_targets.yaml`) or the default value if the setting is not
available. The data type of the return value depends on your choice of
`name`.
...
`tar_config_get()`. `tar_config_get()` retrieves data from an optional
YAML configuration file. You can control the settings in the YAML file

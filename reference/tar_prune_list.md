...
List the targets that
[`tar_prune()`](https://docs.ropensci.org/targets/reference/tar_prune.md)
will remove. Does not actually remove any targets.
...
If `callr_function` is
[`callr::r_bg`](https://callr.r-lib.org/reference/r_bg.html), the return
value is a handle to the `callr` background process is returned.

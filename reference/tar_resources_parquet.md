...
Create the `parquet` argument of
...
- compression:
  ...
  `arrow::write_parquet()`. Defaults to `"snappy"`.

- compression_level:

  Numeric of length 1, `compression_level` argument of
  `arrow::write_parquet()`. Defaults to `NULL`.

## Value

Object of class `"tar_resources_parquet"`, to be supplied to the parquet
argument of
[`tar_resources()`](https://docs.ropensci.org/targets/reference/tar_resources.md).

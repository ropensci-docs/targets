...
download local metadata files to the cloud location (repository, bucket,
...
- strict:
  ...
  Logical of length 1. `TRUE` to error out if the file does not exist in
  the bucket, `FALSE` to proceed without an error or warning. If
  `strict` is `FALSE` and `verbose` is `TRUE`, then an informative
  message will print to the R console.

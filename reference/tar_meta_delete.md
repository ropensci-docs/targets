...
Delete the project metadata files from the local file system, the cloud,
or both.
...
- delete:
  ...
  Character of length 1, which location to delete the files. Choose
  `"local"` for local files, `"cloud"` for files on the cloud, or
  `"all"` to delete metadata files from both the local file system and
  the cloud.
